 ...
 *
 * In order to avoid infinite loops of ephemeral messages, every message carries a session ID, which
 * is a random number generated by the sender at startup time, and a sequence number. Receivers keep
 * the highest sequence number seen per (senderId, sessionId) and ignore anything not greater.
 * */
export type EphemeralMessage = {
  ...
  /** A sequence number, allocated once per broadcast. See {@link EphemeralStamp}. */
  count: number
...
/**
 * The contents of a message, without the sender ID or other properties added by the
 * {@link NetworkSubsystem}. Ephemeral messages already carry their {@link EphemeralStamp}.
 */
 ...
 *
 * Identifies one ephemeral broadcast. Every per-peer copy carries the same
 * stamp, so a receiver sees one message rather than several.
 */
